...
RInChI conversion module.
    ...
    Minor additional material added 2013 N.A. Parker (specifically,
    .rxn to mol file agent conversion and subsequent amendments for
    agents in the .rxn to RInChI converter).
...
This module provides a variety of functions for the interconversion of InChIs,
AuxInfos, Molfiles, RXNfiles and more.


Modified 2013 by N. A . Parker - added support to the rxn2rinchi function for non
standard .rxn files containing reaction agents specified separately from the
reactants and products.

Modified 2014 by B. Hammond - extended support for non standard .rxn files to the
rdf parsing functions.
Modified all .rxn handling functions to no longer discard reaction data in the
...
def rxn_2_molfs(rxn):
    ...
    Returns:
        A tuple of three lists; the first a list of the reactants' molfiles,
        the second a list of the products' and the third a list of the agents'.
        These molfiles are also in the form of strings.
    """
    ...
    # Loop through these sections collecting up the reactants, products and
    # agents.
    reactants = []
...
def split_rdf(rdfile, start=0, stop=0):
    ...
    However, this is not as simple as just splitting the RDFile into its
    constituent RXN entries, for some RXN entries contain additional data
    specifying variations on the core reaction.  Furthermore, each RXN entry
    (and every variation thereof) may have catalysts and solvents saved as
    molfiles embedded within the additional data.
    ...
    # Loop through the RDFile, scanning for variations and extra substances and
...
def rdf_2_rinchis(
        ...
        return_rauxinfos=False,
        return_longkeys=False,
        return_shortkeys=False,
        return_webkeys=False,
        return_rxndata=False):
    """Convert an RDFile to a list of RInChIs.

    Args:
        rdf: The contents of an RDFile as a string.
        start: The index of the RXN entry within the RDFile at which to start
            converting.  If set at default value (0), conversion begins from
            the first RXN entry.
        stop: The index of the RXN entry within the RDFile at which to stop
